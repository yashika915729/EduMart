...
export default function SellPage() {
  ...
  // Manage offer (accept/reject)
  const handleOffer = async (offerId, action) => {
    const toastId = toast.loading(
      `${action === "accept" ? "Accepting" : "Rejecting"} offer...`
    );
    try {
      const res = await fetch("/api/offers/manage", {
        method: "PUT",
      ...
      toast.success(`Offer ${action}ed successfully!`, { id: toastId });
      fetchProducts();
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6 text-center">Sell Your Product</h1>
            ...
            >
              ...
              <div className="flex gap-2 mt-3">
                <button
                  ...
                  className="flex-1 bg-yellow-500 text-white rounded p-1 hover:bg-yellow-600"
                >
                  Edit
                </button>
                <button
                  ...
                  {product.offers.map((offer) => (
                    ...
                    >
                      <p className="text-sm text-gray-700">
                        ₹{offer.price} by {offer.userName}
                      </p>
                      {offer.status === "pending" && (
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleOffer(offer._id, "accept")}
                            className="bg-green-500 text-white text-xs rounded px-2 py-1"
                          >
                            Accept
                          </button>
                          <button
                            onClick={() => handleOffer(offer._id, "reject")}
